...
void LCDShield::clearScreen(int x, int y, int w, int h,int color)
{
	setRect(x,y,x+w,y+h,1,color);	
}
...
void LCDShield::drawImage(int x, int y,char image[],int clearColor,int w,int h){

	int originalX = x;
	char logo;
	for (int logo_ix = 0; logo_ix < w*h; logo_ix++)
	{
		logo = image[logo_ix];
		{
			if ((logo & 0xfff) == 0xfff)
			{
				setPixel(clearColor,x,y);
			} else
			{
				setPixel(logo, x, y);
			}
